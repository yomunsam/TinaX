...
namespace TinaX.VFSKit
{
    public class VFSKit:IVFS
    {
        ...
        /// </summary>
        private readonly string mVFilePersistentDataPath;
        private readonly string mVFileStreamingAssetsPath;
        private readonly VFSConfigModel mConfig;
        ...
        private readonly string mVFileVersionInfoPath; //描述当前VFS资源版本的json文件的路径
        private readonly VFSFileVersionInfoModel mVFileVersionInfo;
        private readonly LoadedAssetBundleMgr mLoadedABMgr;
        private readonly AssetBundleLoadPlanMgr mABLoadPlanMgr;  //异步加载计划管理
        private readonly byte[] mEncrySign = System.Text.Encoding.UTF8.GetBytes(VFSEncryConst.EncryFileSign);    //加密头部标记
        private readonly FileDownloadPlanMgr mDownloadPlanMgr;    //异步 Web下载计划管理

        //======  WEB VFS

        private bool mEnableWebVFS = false;
        private bool mWebVFSIniting = false;
        private UniTask mWebVFSInitTask;
        private bool mWebVFSInited = false;
        private readonly string mVFileWebVFSCachePath;
        ...
        public VFSKit()
        {
            ...
            if (mConfig.FileMode == VFSMode.AssetBundle)
            {
                ...
                //加载基础资源
                void InitForAssetBundleLoad() //为AssetBundle功能执行初始化
                {
                    var streamingAsset_manifest_bundle = AssetBundle.LoadFromFile(mVFileStreamingAssetsPath + Const.PlatformConst.GetPlatformName(Application.platform).ToLower());
                    mStreamingPathAssetBundleManifest = streamingAsset_manifest_bundle.LoadAsset<AssetBundleManifest>("AssetBundleManifest");
                    streamingAsset_manifest_bundle.Unload(false);
                    streamingAsset_manifest_bundle = null;

                    TryInitAssetBundleForPersistenData();
                }
                void TryInitAssetBundleForPersistenData()
                {
                    var persisten_manifest_bundle_path = mVFilePersistentDataPath + Const.PlatformConst.GetPlatformName(Application.platform).ToLower();
                    if (File.Exists(persisten_manifest_bundle_path))
                    {
                        var persisten_manifest_bundle = AssetBundle.LoadFromFile(persisten_manifest_bundle_path);
                        mPersistentDataAssetBundleManifest = persisten_manifest_bundle.LoadAsset<AssetBundleManifest>("AssetBundleManifest");
...
#region AssetBundle Load
        ...
        private AssetBundle LoadAssetBundleAndDependencesRecursion(string ab_path)
        {
            ...
            string[] dependences = GetABDependences(ab_path, loadFromPerPath);
            ...
            //加载ab本体
            int UseNum = 0;
            Hash128 hash_will_load = GetABHashCode(ab_path, loadFromPerPath);
        ...
        private async UniTask<AssetBundle> LoadAssetBundleAndDependencesRecursionAsync(string ab_path)
        {
            ...
            //加载本AB
            int UseNum = 0;
            Hash128 hashCode = GetABHashCode(ab_path, loadFromPerPath);
        ...
        /// <returns></returns>
        private string[] GetABDependences(string ab_path, bool loadFromPerPath)
        {
            string[] result;
            if (loadFromPerPath)
        ...
        /// <param name="loadFromPerPath"></param>
        /// <returns></returns>
        private UnityEngine.Hash128 GetABHashCode(string ab_path, bool loadFromPerPath)
        {
            Hash128 result;
            if (loadFromPerPath)
            {
...
#region Resources Load
        ...
        {
            var manifest = await GetWebVFSAssetBundleManifestAsync();
        ...
        private async UniTask<AssetBundleManifest> GetWebVFSAssetBundleManifestAsync() 
        {
            if (mWebVFSAssetBundleManifest != null)
            {
            ...
            else
            {
                ...
                if (download_flag)
                {
                    ...
                    await DownloadFileToLocal(uri, local_path);
                    ...
                    if (File.Exists(local_path))
                    {
                        ...
                        {
                            throw new Exceptions.VFSException("download failed: hash error", Exceptions.VFSException.VFSErrorType.HashMismatch);
                        }
                    }
        ...
        {
            return (await GetWebVFSAssetBundleManifestAsync()).GetAssetBundleHash(ab_path);
        }
        ...
        private async UniTask<byte[]> DownloadFileFromWebOnce(System.Uri uri)
        {
            //是否正在下载
        ...
        private async UniTask<string> GetTextFromWebOnce(System.Uri uri)
        {
            //是否正在下载
            if (mDownloadPlanMgr.ContainsUri(uri))
            {
        ...
        private async UniTask DownloadFileToLocal(System.Uri uri,string local_path)
        {
            if (!File.Exists(local_path))
            {
                try
                {
                    var data = await DownloadFileFromWebOnce(uri);
                    if (!File.Exists(local_path))
                    {
        ...
        private async UniTask<VFSFileHashModel> GetWebVFS_FileHashList()
        {
            ...
            else
            {
                ...
                var json_str = await GetTextFromWebOnce(uri);
        ...
        /// <summary>
        /// 在构造函数中的WebVFS的初始化部分 中的异步部分
        /// </summary>
        /// <returns></returns>
        private async UniTask InitWebVFSAsync()
        {
            //MD5 List
            if (mCurWebVFSConfig.VerifyFileHash)
            {
            ...
            try
            {
                await GetWebVFSAssetBundleManifestAsync();
            }
        ...
        private void InitAssetBundleDataForTinaXWorkFolder()    //用来直接从TinaX Work Folder中加载AssetBundle的初始化项目
        {
            ...
            mTinaXWorkFolderAssetBundleManifest = manifest_bundle.LoadAsset<AssetBundleManifest>("AssetBundleManifest");
        }
